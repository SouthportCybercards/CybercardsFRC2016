    ...
    void AutonomousPeriodic()
    {
        if(autoSelected == lowBarAuto){
            ...
            if(currentTime < 2.0){
                ...
                /*}else if(currentTime < 4.5){ //pew pew pew
            ...
            float currentTime = autoTimer.Get();bool frontGreen = CheckSensorForGreen(FRONT); //ColorSensors[FRONT], FRONT);
            bool backGreen = CheckSensorForGreen(BACK);
            bool back2Green = CheckSensorForGreen(BACK2);
            bool overGreenPrevious = overGreen;
            if(frontGreen && backGreen && back2Green){ //checking all sensors for this
...
void TeleopPeriodic()
{
    ...
    void TestColorSensor(){
        ...
        if(currentTime == 0.0){
            ...
            tcaselect(FRONT);
            }else if(ColorSensorTimer.HasPeriodPassed((float)0.7)){ //change this to shorter time later
            if(currentTime > 10){
